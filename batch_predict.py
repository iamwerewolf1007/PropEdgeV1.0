"""
PropEdge V1.0 — batch_predict.py  (B1-B5)
...
from config import (
    ...
    FILE_ELITE_MODEL, FILE_V1_MODEL, ODDS_API_KEY, ODDS_BASE_URL,
    ELITE_THRESHOLDS, ELITE_STAKES, ELITE_TIER_NUM, ELITE_TIER_LABEL,
    ELITE_FEATURES, V1_FEATURES, TRUST_THRESHOLD, MIN_PRIOR_GAMES,
...
class MS:
    ...
    @property
    def elite(self):return self._p(FILE_ELITE_MODEL)   # legacy GBT fallback
    @property
    def v1(self):  return self._p(FILE_V1_MODEL)     # V1.0 primary model
    @property
...
def _score_fallback(ev) -> float:
    """Hardcoded fallback when no model is available at all."""
    ...
    if (p12 > 0.5) == (p14 > 0.5):     bonus += abs(p14-0.5)*2 * 0.04
    return float(np.clip(base + bonus, 0.40, 0.81))


def score_elite(ev, play: dict | None = None) -> float:
    """
    V1.0 scoring function.

    Tries models in priority order:
      1. V1.0 XGB+LGB+LR ensemble (propedge_v1.pkl) — PRIMARY
         Uses 102 features (82 legacy + 20 new).
         Target: P(Bet Win) = P(direction correct).
         Tier thresholds: APEX>=0.90, ULTRA>=0.85, ELITE>=0.78, STRONG>=0.72, PLAY+>=0.65
      2. V1.0 legacy GBT (propedge_v1_legacy.pkl) — LEGACY FALLBACK
         Used if V1.0 model not yet trained (first run before retrain).
         Note: V1.0 legacy model was trained on raw WIN target, so probabilities are
         not directly comparable to V1.0 thresholds. A 0.05 offset is applied
         to compensate for the threshold shift.
      3. Hardcoded formula — EMERGENCY FALLBACK
         Used if neither model file exists.

    Parameters
    ----------
    ev   : dict built by build_ev() — 82 ELITE_FEATURES
    play : raw prop dict — provides odds, min/max line, usage, h2h_conf for V1.0 features
    """
    play = play or {}

    # ── Priority 1: V1.0 ensemble ──────────────────────────────────────────────
    pkg1 = M.v1
    if pkg1 is not None:
        try:
            # Build full 102-feature vector
            v1_feats = build_v1_features(ev, play)
            row = {**ev, **v1_feats}

            X = pd.DataFrame(
                [{k: float(row.get(k, 0.0)) for k in pkg1["features"]}]
            )[pkg1["features"]].fillna(0).values

            X_s = pkg1["scaler"].transform(X)
            p_xgb = float(pkg1["xgb"].predict_proba(X)[0, 1])
            p_lgb = float(pkg1["lgb"].predict_proba(X)[0, 1])
            p_lr  = float(pkg1["lr"].predict_proba(X_s)[0, 1])
            prob  = 0.40 * p_xgb + 0.40 * p_lgb + 0.20 * p_lr
            return float(np.clip(prob, 0.0, 1.0))
        except Exception as e:
            print(f"  ⚠ V1.0 model error: {e} — trying legacy fallback")

    # ── Priority 2: V1.0 legacy GBT (legacy fallback) ───────────────────────
    pkg16 = M.elite
    if pkg16 is not None:
        try:
            X = pd.DataFrame(
                [{k: ev.get(k, 0.0) for k in pkg16["features"]}]
            )[pkg16["features"]].fillna(0).values
            raw = float(np.clip(
                pkg16["model"].predict_proba(pkg16["scaler"].transform(X))[0, 1],
                0.0, 1.0
            ))
            # V16 model was trained on raw WIN (not bet-win) so shift calibration
            # by -0.05 to keep tier assignments roughly comparable to V1.0 thresholds
            return float(np.clip(raw - 0.05, 0.0, 1.0))
        except Exception as e:
            print(f"  ⚠ Legacy fallback error: {e}")

    # ── Priority 3: hardcoded formula ─────────────────────────────────────────
    return _score_fallback(ev)
...
def run_batch(date_str):
    ...
    for a in ("v92","v10","v11","v12r","v12c","v12a","v12s","v12q","v14r","v14c","v14a","elite","v1"):
        getattr(M,a)
    if M.v1 is not None:
        feats = getattr(M.v1, "get", lambda k, d: d)("features", [])
        print(f"  ✓ V1.0 model active ({len(feats)} features)")
    elif M.elite is not None:
        print(f"  ⚠ V1.0 model not found — using V1.0 legacy GBT")
        print(f"    Run 'python3 run.py retrain' to build V1.0 model")
    else:
        print(f"  ⚠ No meta-model found — using hardcoded fallback")
        print(f"    Run 'python3 run.py retrain' to build V1.0 model")
    ...
    for prop in props:
        ...
        ep=score_elite(ev,prop); et=assign_elite_tier(ep)
