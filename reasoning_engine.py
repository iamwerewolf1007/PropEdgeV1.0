"""
PropEdge V1.0 — reasoning_engine.py
...
def _f(v, default: float = 0.0) -> float:
    """Safe float conversion — returns default if value is None or not numeric."""
    try:
...
def generate_pre_match_reason(play: dict) -> str:
    ...
    line      = _f(play.get("line"), 20)
    direction = play.get("direction", "OVER") or "OVER"
    tier      = play.get("tierLabel", "STRONG") or "STRONG"
    pred_pts  = play.get("predPts")
    pred_gap  = _f(play.get("predGap"), 0)
    conf      = _f(play.get("conf"), 0.6)
    flags     = _i(play.get("flags"), 5)
    flag_d    = play.get("flagDetails") or []

    L30   = _f(play.get("l30"),   line)
    L10   = _f(play.get("l10"),   L30)
    L5    = _f(play.get("l5"),    L10)
    L3    = _f(play.get("l3"),    L5)
    std10 = _f(play.get("std10"), 5)
    hr10  = _f(play.get("hr10"),  0.5)
    hr30  = _f(play.get("hr30"),  0.5)
    ...
    opp      = play.get("opponent", "") or ""
    dvp      = _i(play.get("defP_dynamic"), 15)
    pace     = _i(play.get("pace_rank"),    15)
    h2h_avg  = _f(play.get("h2h_avg"),    0)
    h2h_g    = _i(play.get("h2h_games"),  0)
    h2h_ts   = _f(play.get("h2h_ts_dev"), 0)

    early_w  = _f(play.get("early_season_weight"), 1.0)
    mean_rev = _f(play.get("mean_reversion_risk"), 0.0)
    ...
    disagree_names = [fd["name"] for fd in flag_d if not fd.get("agrees")]
    if flags >= 8:
    ...
    if pred_pts is not None:
        s4 = f"V1.0 model projects {pred_pts:.1f}pts ({'+' if pred_gap >= 0 else ''}{pred_gap:.1f} vs line). Fusion confidence: {conf:.0%} [{tier}]."
        if early_w < 0.5:
    ...
    elif _f(play.get("is_long_rest"), 0) == 1:
...
def generate_post_match_reason(play: dict, box_data: Optional[dict] = None) -> tuple[str, str]:
    ...
    box = box_data or {}
    actual_pts  = _f(box.get("actual_pts",  play.get("actualPts")),  0)
    actual_min  = _f(box.get("actual_min",  play.get("actualMin")),  0)
    actual_fga  = _f(box.get("actual_fga"),  0)
    actual_fgm  = _f(box.get("actual_fgm"),  0)
    actual_fg   = actual_fgm / max(actual_fga, 1)

    l10_min = _f(play.get("min_l10"), 30)
    l10_fga = _f(play.get("fga_l10"), 10)
    l10_fg  = _f(play.get("l10_fg_pct"), 0.45)
    flags   = _i(play.get("flags"), 5)
    momentum = _f(play.get("l5"), line) - _f(play.get("l30"), line)
