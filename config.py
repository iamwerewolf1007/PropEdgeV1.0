"""
PropEdge V1.0 — config.py
All constants, paths, Elite tier thresholds, clean_json helper.

V1.0 changes vs V1.0:
  - VERSION bumped to V1.0
  - ELITE_THRESHOLDS recalibrated from holdout validation (Feb-Apr 2026):
      APEX  >=0.90 -> 96.9% HR  (was >=0.81 / 92%)
      ULTRA >=0.85 -> 93.6% HR  (was >=0.78 / ~80%)
      ELITE >=0.78 -> 87.6% HR  (was >=0.75 / ~69%)
      STRONG>=0.72 -> 81.3% HR  (was >=0.72 / ~66%)
      PLAY+ >=0.65 -> 76.5% HR  (was >=0.68 / ~59%)
  - ELITE_STAKES updated to match new tier performance
  - V1_NEW_FEATURES (20): cross-interaction + market intelligence features
    added on top of existing 82 ELITE_FEATURES -> 102-feature V1.0 model
  - FILE_V1_MODEL added (models/elite/propedge_v1.pkl)
  - Model bundle: {xgb, lgb, lr, scaler, features, ...}
    score_elite() uses XGB(40%)+LGB(40%)+LR(20%) ensemble
  - Training target corrected: Bet Win = direction was correct
    (not raw result=='WIN' which ignored UNDER bet direction)
"""
...
FILE_ELITE_MODEL = MODEL_DIR / "elite" / "propedge_v1_legacy.pkl"  # legacy GBT fallback
FILE_V1_MODEL   = MODEL_DIR / "elite" / "propedge_v1.pkl"       # V1.0 primary model
...
GIT_REMOTE    = "git@github.com:iamwerewolf1007/PropEdgeV1.0.git"
LOCAL_DIR     = Path.home() / "Documents" / "GitHub" / "PropEdgeV1.0Local"
REPO_DIR      = Path.home() / "Documents" / "GitHub" / "PropEdgeV1.0"
...
# Elite tier system — V1.0 recalibrated thresholds
# Validated on true holdout Feb-Apr 2026 (n=5,807 plays, AUC=0.800)
# Previous thresholds in parentheses for reference
ELITE_THRESHOLDS = {
    "APEX":   0.90,   # (prev: 0.81) holdout HR 96.9% vs 92.0%
    "ULTRA":  0.85,   # (prev: 0.78) holdout HR 93.6% vs ~80%
    "ELITE":  0.78,   # (prev: 0.75) holdout HR 87.6% vs ~69%
    "STRONG": 0.72,   # (prev: 0.72) holdout HR 81.3% vs ~66%
    "PLAY+":  0.65,   # (prev: 0.68) holdout HR 76.5% vs ~59%
}
# Stakes updated to reflect higher validated HR at each tier
ELITE_STAKES     = {"APEX":4.0,"ULTRA":3.0,"ELITE":2.0,"STRONG":1.5,"PLAY+":1.0,"SKIP":0.0}
...
def assign_elite_tier(prob: float) -> str:
    if prob >= 0.90: return "APEX"
    if prob >= 0.85: return "ULTRA"
    if prob >= 0.78: return "ELITE"
    if prob >= 0.72: return "STRONG"
    if prob >= 0.65: return "PLAY+"
    return "SKIP"
...
# V14 ML feature list (56) — unchanged
V14_ML_FEATURES = [
...
# Elite meta-model features (82) — core feature set
ELITE_FEATURES = [
...
# V1.0 NEW features (20) — validated top features from ML research
# These 20 new features drove AUC from ~0.61 to 0.80 in holdout testing
V1_NEW_FEATURES = [
    # Cross-interaction signals (most impactful category)
    "v1_inqband_x_gap",       # line_in_q x gap_mean_real — double confirmation
    "v1_momentum_x_dvp",      # (L5-L30) x (31-defP_dynamic) — hot player vs weak D
    "v1_std10_x_line_z",      # std10 x |line_vs_avg_z| — volatile + misset line
    "v1_usage_x_pace",        # usage_l10 x (31-pace_rank) — usage amplified by pace
    "v1_rest_x_form",         # rest_days x form_hot — rested + hot player
    "v1_hr10_x_conv",         # hr10 x v12_clf_conv — recent form x model conviction
    # Quantile uncertainty
    "v1_q_width_pct",         # q_range / line — narrower = more confident
    "v1_q_coverage",          # 1 - q_range/20 — tightness of prediction band
    # Market edge signals
    "v1_model_vs_market",     # prob_v12 - implied_over_prob — model vs bookmaker
    "v1_juice_total",         # implied_over + implied_under prob — vig indicator
    "v1_line_sharpness",      # min_line / max_line — book agreement (needs min/max)
    # Form state indicators
    "v1_form_hot",            # L3>L5>L10 — accelerating upward streak
    "v1_form_cold",           # L3<L5<L10 — accelerating downward streak
    "v1_recency_hr",          # (3*hr10 + hr30) / 4 — recency-weighted hit rate
    "v1_is_starter",          # minL10 >= 28 — starter (stable role)
    # Gap features
    "v1_gap_abs",             # abs(gap_mean_real) — magnitude of model-line disagreement
    "v1_pred_gap_sq",         # gap_mean_real^2 — non-linear gap effect
    "v1_all_gaps_same_dir",   # all sub-model gaps same sign — unanimous direction
    # Non-linear season effect
    "v1_season_sq",           # season_progress^2 — non-linear mid-season dip
    # H2H quality
    "v1_h2h_weighted",        # h2h_avg_gap x h2h_conf — confidence-weighted H2H
]

# Full V1.0 feature list: 82 existing + 20 new = 102 total
V1_FEATURES = ELITE_FEATURES + V1_NEW_FEATURES
