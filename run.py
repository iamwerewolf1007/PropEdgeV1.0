...
Commands:
  ...
  python3 run.py retrain                     — Retrain V1.0 XGB+LGB+LR ensemble (+ trust scores)
...
def cmd_generate() -> None:
    ...
    print("\n  [2/2] Training V1.0 XGB+LGB+LR ensemble...")
...
# ─────────────────────────────────────────────────────────────────────────────
# RETRAIN  (manual trigger — retrain V1.0 ensemble meta-model only)
# ─────────────────────────────────────────────────────────────────────────────
def cmd_retrain() -> None:
    print(f"\n  PropEdge {VERSION} — Retrain V1.0 Ensemble")
    _run("model_trainer.py")
...
def cmd_sync() -> None:
    ...
    GitHub folder (PropEdgeV1.0). Run this before committing via GitHub Desktop.
    ...
    if not REPO_DIR.exists():
        ...
        print(f"  Extract PropEdgeV1.0_GitHub.zip to {REPO_DIR.parent} first.")
        return
