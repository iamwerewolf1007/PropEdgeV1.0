...
Actions:
  ...
  6. Monthly trigger: retrain V1.0 Ensemble (XGB+LGB+LR) on all 2025-26 graded plays
  7. Git push
...
def grade_plays(
    ...
    for p in plays:
        ...
        # ── Grade ─────────────────────────────────────────────────────────
        # Always grade as OVER: WIN = actual > line, LOSS = actual <= line.
        # The Props Excel is an OVER bet database. Using model-predicted direction
        # for grading causes results to change on every generate/retrain cycle.
        # dir_ is preserved on the play for display purposes only.
