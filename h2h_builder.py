...
def build_h2h(
    ...
    gl["DNP"] = gl.get("DNP", 0) if isinstance(gl.get("DNP", 0), int) else gl.get("DNP", pd.Series(0, index=gl.index))
    ...
    for (player, opp), grp in played.groupby(["PLAYER_NAME", "OPPONENT"]):
        ...
        # Home/away
        home_games = grp[grp["IS_HOME"].isin([1, True, "True", "1"])]
        away_games = grp[grp["IS_HOME"].isin([0, False, "False", "0"])]
